...
* As defined by the EJB specification, stateless and singleton session beans are not passivation capable dependencies.
...
* a non-transient field which is annotated with +@TransientReference+, or
* a non-transient field which resolves to a bean that is a passivation capable bean, or
* a bean constructor parameter which is annotated with +@TransientReference+, or
* a bean constructor parameter which resolves to a bean that is a passivation capable bean, or
* a method parameter which is annotated with +@TransientReference+, or
* a method parameter which resolves to to a bean that is a passivation capable bean,
...
* a managed bean which declares a passivating scope type, or
* a stateful session bean which declares a passivating scope type, or
* a built-in bean

has a decorator or interceptor which is not a passivation capable dependency, the container automatically detects the problem and treats it as a deployment problem.

If a custom implementation of +Bean+ which declares a passivation scope type is not a passivatioon capable bean the container automatically detects the problem and treats it as a deployment problem.
